...
(define-record coops-instance class slots)
...
(define (slot-exists? obj slotname)
  (and (slot-lookup obj slotname 'slot-initialized?) #t))
...
(define-syntax call-next-method-lambda
  (er-macro-transformer
   (lambda (x r c)
     (let ((llist (cadr x))
           (body (cddr x)))
...
(define-syntax ensure-generic-procedure
  (er-macro-transformer
   (lambda (x r c)
     (let* ((args (cadr x))
            (name (caddr x))
...
(define-syntax register-generic-procedure
  (er-macro-transformer
   (lambda (x r c)
     (let ((name (cadr x)))
       (when (symbol? name)
